...
public abstract class AbstractBlobStoreService {
    ...
    /**
     * Return the content of a blob, or {@code null} if it does not exist, downloading it only once.
     */
    public byte[] getBlobContent(@Header(value = Constants.FILE_HANDLE) String name) {
        return repository.getBlobContent(name);
    }
