 ...
 * clients. See {@link AdminRestRouteBuilder} for the internal API used to interact with front-ends.
 */
@Component
public class AdminExternalRestRouteBuilder extends BaseRouteBuilder {
    ...
    public AdminExternalRestRouteBuilder(
    ...
    public void configure() throws Exception {
        ...
        restConfiguration()
                .component("servlet")
                .contextPath("/services-external")
                .bindingMode(RestBindingMode.json)
                .endpointProperty("matchOnUriPrefix", "true")
                .apiContextPath("/openapi.json")
                .apiProperty("api.title", "Timetable Management External API").apiProperty("api.version", "1.0");

        String commonApiDocEndpoint = "http:" + host + ":" + port + "/services-external/openapi.json?bridgeEndpoint=true";

        rest("/timetable_admin_external")
                ...
                .get("/download_private_dataset/{codespace}")
                .description("Download NeTEx dataset with private data")
                .param().name("codespace").type(RestParamType.path).description("Codespace of the organization producing the NeTEx dataset with private data").dataType(OPENAPI_DATA_TYPE_STRING).endParam()
                .consumes(PLAIN)
        ...
        from("direct:adminExternalUploadFile")
                ...
                .process(e -> e.getIn().setBody(new UploadResult(e.getIn().getHeader(Constants.CORRELATION_ID, String.class))))
                .marshal().json()
    ...
    private record UploadResult(String correlationId) {}
