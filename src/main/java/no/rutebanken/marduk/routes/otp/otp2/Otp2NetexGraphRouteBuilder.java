...
public class Otp2NetexGraphRouteBuilder extends BaseRouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        from("direct:remoteBuildOtp2NetexGraphAndSendStatus")
                ...
                .process(e -> JobEvent.systemJobBuilder(e).jobDomain(JobEvent.JobDomain.GRAPH).timetableAction(JobEvent.TimetableAction.OTP2_BUILD_GRAPH).state(JobEvent.State.FAILED).correlationId(e.getProperty(TIMESTAMP, String.class)).build()).to("direct:updateStatus")
        ...
        from("direct:remoteOtp2GraphPublishing")
                ...
                .process(e -> JobEvent.systemJobBuilder(e).jobDomain(JobEvent.JobDomain.GRAPH).timetableAction(JobEvent.TimetableAction.OTP2_BUILD_GRAPH).state(JobEvent.State.OK).correlationId(e.getProperty(TIMESTAMP, String.class)).build())
        ...
        from("direct:sendOtp2NetexGraphBuildStartedEventsInNewTransaction")
                ...
                .process(e -> JobEvent.systemJobBuilder(e).jobDomain(JobEvent.JobDomain.GRAPH).timetableAction(JobEvent.TimetableAction.OTP2_BUILD_GRAPH).state(JobEvent.State.STARTED).correlationId(e.getProperty(TIMESTAMP, String.class)).build()).to("direct:updateStatus")
