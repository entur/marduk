...
public class GraphPublishRouteBuilder extends BaseRouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        from("direct:notifyEtcd")
                ...
                .choice()
                .when(header("notificationUrl").isNotEqualTo("none"))
                .log(LoggingLevel.INFO, getClass().getName(),  "Notifying " + header("notificationUrl") + " about new otp graph.")
                .process(e -> e.getIn().setBody("value=" + e.getIn().getHeader(FILE_HANDLE, String.class)))
                .removeHeaders("*")
                .setHeader(Exchange.HTTP_METHOD, constant(HttpMethods.PUT))
                .setHeader(Exchange.CONTENT_TYPE, constant("application/x-www-form-urlencoded"))
                .to("log:" + getClass().getName() + "?level=DEBUG&showAll=true&multiline=true")
                .toD("${header.notificationUrl}")
