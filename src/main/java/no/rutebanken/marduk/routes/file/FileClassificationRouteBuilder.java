...
public class FileClassificationRouteBuilder extends BaseRouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        from("entur-google-pubsub:ProcessFileQueue")
                ...
                .process( e -> e.getIn().setHeader(Constants.CHOUETTE_JOB_FAILURE_CODE, "JOB_FAILURE_UNKNOWN_FILE_EXTENSION"))
                ...
                .process( e -> e.getIn().setHeader(Constants.CHOUETTE_JOB_FAILURE_CODE, "JOB_FAILURE_UNKNOWN_FILE_TYPE"))
                .process(e -> JobEvent.providerJobBuilder(e).timetableAction(JobEvent.TimetableAction.FILE_TRANSFER).state(JobEvent.State.FAILED).build())
                .to("direct:updateStatus")
                .stop()
                ...
                .process( e -> e.getIn().setHeader(Constants.CHOUETTE_JOB_FAILURE_CODE, "JOB_FAILURE_NOT_A_ZIP_FILE"))
                .process(e -> JobEvent.providerJobBuilder(e).timetableAction(JobEvent.TimetableAction.FILE_TRANSFER).state(JobEvent.State.FAILED).build())
                .to("direct:updateStatus")
                .stop()
                ...
                .process( e -> e.getIn().setHeader(Constants.CHOUETTE_JOB_FAILURE_CODE, "JOB_FAILURE_INVALID_XML_ENCODING"))
                .process(e -> JobEvent.providerJobBuilder(e).timetableAction(JobEvent.TimetableAction.FILE_TRANSFER).state(JobEvent.State.FAILED).build())
                .to("direct:updateStatus")
                .stop()
                ...
                .process( e -> e.getIn().setHeader(Constants.CHOUETTE_JOB_FAILURE_CODE, "JOB_FAILURE_INVALID_ZIP_FILE_ENTRY_ENCODING"))
                .process(e -> JobEvent.providerJobBuilder(e).timetableAction(JobEvent.TimetableAction.FILE_TRANSFER).state(JobEvent.State.FAILED).build())
                .to("direct:updateStatus")
                .stop()
